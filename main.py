...
# 4. 총 월 구독 비용 계산 함수
# 수정: sum(prices)==0 → len(prices)==0 으로 변경
# 수정: global total_cost 선언 추가
...
# 6. 예산 초과 여부 확인 함수
# 수정: platforms를 파라미터로 추가
def bud_check(month_bud, prices, platforms):
    if month_bud == 0:
    ...
    if len(prices) == 0:
        ...
        if month_bud * len(platforms) > total:
            print("월 구독 비용 예산은 초과되지 않았습니다.")
        elif month_bud * len(platforms) == total:
            print("월 구독 비용 예산과 동일합니다. 더 이상 구독 플랫폼을 추가하면 초과합니다.")
        else:
            print("월 사용 가능 구독 비용을 초과하였습니다. 구독 플랫폼을 줄이세요.")
...
while True:
    ...
    if check == 1:
        ...
        bud_check(month_bud, prices, platforms)  
