...
* 이 프로그램은 사용자가 자신이 이용 중인 정기 구독 서비스와 월 구독 비용을 관리할 수 있도록 만든 콘솔 프로그램입니다. 사용자는 월에 사용할 수 있는 예산을 먼저 입력하고, 이후 자신이 구독 중인 플랫폼 이름과 월 이용 금액을 입력할 수 있습니다. 시스템은 입력된 모든 구독 서비스의 월 비용을 합산하여 총 월 구독 비용을 계산합니다. 또한 계산된 총 비용이 사용자가 입력한 월 사용 가능 금액을 초과하는지 비교하여, 예산 초과 여부를 알려줍니다. 예산을 초과한 경우에는 "사용가능 금액을 초과합니다. 줄여야 합니다." 라고 출력하고 초과하지 않은 경우에는  "사용가능 금액을 초과하지 않습니다." 라고 출력하도록 구성할 계획입니다. 반복문과 메뉴를 이용하여 구독 서비스 추가, 전체 목록 조뢰, 총 비용 계산, 특정 서비스 삭제 등의 기능을 수행할 수 있습니다. 최종적으로는 배열과 함수를 활용하여 여러 개의 구독 정보를 저장하고, 사용자의 소비 습관을 간단히 관리할 수 있는 프로그램으로 확장할 계획입니다.
...
### **🤖 AI 파트너십 과정**
 ...
 1. **내용 1: **콘솔 프로그램에 대한 포괄적 주제 추천**
    * **프롬프트 요약:** ""나는 C언어 기초를 배우는 대학생이야. 
    * **적용 내용:**
      
 2. **내용 2 : **정기 구독 관리 시스템이 지금까지 배운 파이썬 내용을 잘 반영하는지**
    * **프롬프트 요약:** ""나는 사용자가 정기 구독 서비스 이름과 월 비용을 입력하면 이를 저장하고 관리하는 프로그램인 정기 구독 관리 시스템을 하고 싶어. 저장된 구독들의 월 총 지출을 계산하여 사용자가 설정한 예산과 비교하는 거지. 너의 나의 아이디어를 보고 지금까지 배운 씨언어 내용으로 구현하기 어렵다면, 나의 전체 흐름은 잃지 않게 조금 쉽게 변형해서 보여줘. ""
